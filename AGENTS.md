...
**Generated:** 2026-06-30 (refreshed)
**VCS:** not a git repo
**Phase:** implementation complete — 33/33 todos done, 629 tests, 53M row DB, direction pending BLCM+V-World keys
...
```
부동산호수알아내기/
├── src/                    # 31개 Python 소스 모듈
│   ├── channels/           # 9개 채널 (base/juso/kapt/onbid/aptbg/rtms/auction/registry/lh_vacancy)
│   ├── ingest/             # 7개 ETL (public_price/collective_building/housing_permit/building_registry/reference_tables/unit_master_refine)
│   ├── domain.py           # Listing/Evidence/HoConclusion/Provenance
│   ├── ho_key.py           # canonical ho_id 정규화
│   ├── fusion.py           # Fellegi-Sunter 매칭
│   ├── pipeline.py         # Dempster-Shafer 결합
│   ├── matcher.py          # match_ho()
│   ├── floorplan_infer.py  # 전유부 역추론 배치도
│   ├── inference.py        # infer_complex + infer_unit
│   ├── ground_truth.py     # 정답 라벨 다원화
│   ├── tracker.py          # 호별 상태 추적
│   ├── dictionary.py       # line_fact 사전
│   ├── realtime.py         # Realtime 구독
│   ├── report.py / kpi.py  # 리포트 + KPI
│   ├── db.py               # Supabase 클라이언트
│   ├── local_db.py          # SQLite 로컬 DB (Supabase 호환)
│   └── api_public_price.py  # 공시가격 API 클라이언트
├── scripts/                # 13개 유틸리티 스크립트
│   ├── view_ho.py          # 2D 호수 추론 시각화
│   ├── view_ho3d.py        # 3D 건물 시각화 (V-World/Cesium)
│   ├── build_line_fact.py  # line_fact 구축 (모든 단지)
│   ├── load_*.py           # 데이터 적재 (public_price/housing_permit/registry)
│   ├── infer_pp.py         # 공시가격 기반 추론 테스트
│   └── view_building.py    # V-World 건물 폴리곤 시각화
├── tests/                  # 629개 테스트 (pytest)
├── supabase/migrations/    # SQL 마이그레이션 (Supabase 규격)
├── local.db                # SQLite DB (53M+ rows)
...
| DB 조회 / 테스트 | `local.db` (53M행) | `python scripts/view_ho.py "래미안원베일리" 114 25 84L 남향` |
| line_fact 구축 | `scripts/build_line_fact.py` | 모든 단지 라인→방향 매핑 생성 |
| 배치도/평면도 API | BLCM (blcm.go.kr) | `DrawSearch.do` — 키 발급 필요 |
| 건물 폴리곤 API | V-World (운영심사 중) | `LT_C_BLDGINFO` — direction 교차검증 |
| 건축HUB API | `PUBLIC_DATA_API_KEY` | 기본개요/층별개요/전유부 등 |
